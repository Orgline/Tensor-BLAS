...
int main(int argc,char *argv[])
{
    ...
    startTimer();
    tc_cumpsgemm_trmm(cumpsgemm_handle, m, n, alpha, A, m, B, m, C, m, nb);
    ms = stopTimer();
    //printMatrixDeviceBlock("tC.csv", m, n, C, m);
    
    printf("tc_cumpsgemm_trmm %dx%d takes %f ms, flops is %f\n", m, n, ms, 1.0*m*n*m/ms/1e9);
    ...
    //printMatrixDeviceBlock("tC.csv", m, n, C, m);
        ...
        printf("tc_cublas_trmm %dx%d takes %f ms, flops is %f\n", m, n, ms, 1.0*m*n*m/ms/1e9);
